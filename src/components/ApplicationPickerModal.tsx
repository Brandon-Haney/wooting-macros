...
/** Picks application executables: running programs, installed Steam games, or a file. */
export default function ApplicationPickerModal({
  ...
  const toggleExe = useCallback(
    (exe: string) => {
      ...
      if (trimmed === '') return
      if (isLinked(trimmed)) {
        onChange(linked.filter((l) => !sameExe(l, trimmed)))
      } else {
        onChange([...linked, trimmed])
      }
    },
    [isLinked, linked, onChange]
  )
  ...
  const browseForExe = useCallback(async () => {
    try {
      ...
      if (name && !isLinked(name)) onChange([...linked, name])
    } catch (e) {
      error(String(e))
    }
  }, [isLinked, linked, onChange])

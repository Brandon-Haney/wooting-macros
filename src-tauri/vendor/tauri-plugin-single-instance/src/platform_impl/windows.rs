...
unsafe extern "system" fn single_instance_window_proc<R: Runtime>(
    ...
    // Messages such as WM_NCCREATE arrive before the user data is set: nothing to do for them.
    // (Dereferencing the null pointer here is what made debug builds abort on recent Rust.)
    if data_ptr.is_null() {
